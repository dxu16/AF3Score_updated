...
wait_for_jobs() {
  ...
  log_info "Waiting for ${step_name} jobs to complete..."
...
for subfolder in "$af3_input_batch/pdb"/*; do
  if [[ -d "$subfolder" ]]; then
    ...
    # [[ -n "$slurm_partition" ]] && sbatch_args+=(-p "$slurm_partition")
    # [[ -n "$slurm_nodelist" ]] && sbatch_args+=(-w "$slurm_nodelist")
    sbatch_args+=(--partition ica100)
    sbatch_args+=(--account jgray21_gpu)
    sbatch_args+=(--time 72:00:00)
    sbatch_args+=(--qos qos_gpu)
    job_id=$(sbatch "${sbatch_args[@]}" -o "$jax_log_dir/${folder_name}-%j.out" "$pipeline_script_dir/02_submit_prepare_jax.sh" "$subfolder" "$output_dir_jax/$folder_name" "$pipeline_script_dir" "$PYTHON_EXEC")
...
for subfolder in "$af3_input_batch/pdb"/*; do
  if [[ -d "$subfolder" ]]; then
    ...
    # Using comm to compare input structures and H5 file lists
    missing_files_str=$(comm -23 \
      <(find "$pdb_dir" -maxdepth 1 \( -name "*.pdb" -o -name "*.cif" \) -exec basename {} \; | sed 's/\.pdb$//; s/\.cif$//' | sort) \
    <(find "$h5_dir" -maxdepth 1 -name "*.h5" -exec basename {} .h5 \; | sort))
    
    mapfile -t missing < <(printf "%s" "$missing_files_str")
    num_pdb_files=$(find "$pdb_dir" -maxdepth 1 \( -name "*.pdb" -o -name "*.cif" \) | wc -l)
...
for subfolder in "$af3_input_batch/json"/*; do
  if [[ -d "$subfolder" ]]; then
    ...
    sbatch_args+=(--time 72:00:00)
    sbatch_args+=(--qos qos_gpu)
    job_id=$(sbatch "${sbatch_args[@]}" -o "$af3score_log_dir/${folder_name}-%j.out" "$pipeline_script_dir/03_submit_af3score.sh" "$af3_input_batch/json/$folder_name" "$af3_input_batch/jax/$folder_name" "$output_dir_af3score" "$PYTHON_EXEC" "$pipeline_script_dir")
    log_info "--> Job submitted with ID: $job_id"
...
expected_count=$(find "$input_pdb_dir" -maxdepth 1 \( -name "*.pdb" -o -name "*.cif" \) 2>/dev/null | wc -l)
